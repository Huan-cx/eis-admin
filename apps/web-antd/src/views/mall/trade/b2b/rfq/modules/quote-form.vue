...
import { useQuickQuoteFormSchema, useQuickQuoteItemsColumns } from '../data';
...
const [Modal, modalApi] = useVbenModal({
  title: $t('trade.b2b.rfq.actions.quote'),
  async onConfirm() {
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    if (data?.id) {
      const rfqDetail = await getRfq(data.id);

      formApi.setState({ schema: useQuickQuoteFormSchema(rfqDetail.items) });
...
const [Grid, gridApi] = useVbenVxeGrid({
  gridOptions: {
    columns: useQuickQuoteItemsColumns(),
...
<template>
  <Modal v-bind="$attrs">
    ...
    <div v-if="tableData.length > 0" class="mt-6">
      <h4 class="mb-4 font-semibold">{{ $t('trade.b2b.rfq.detail.items') }}</h4>
      <Grid>
